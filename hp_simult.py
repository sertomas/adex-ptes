...
def exergy_analysis_hp(t0, p0, df):
    ...
    ef_ihx = df.loc[31, 'm [kg/s]'] * (df.loc[32, 'e^PH [kJ/kg]'] - df.loc[33, 'e^PH [kJ/kg]'])
    ef_cond = df.loc[31, 'm [kg/s]'] * (df.loc[31, 'e^PH [kJ/kg]'] - df.loc[32, 'e^PH [kJ/kg]'])
    ef_comp = df.loc[31, 'm [kg/s]'] * (df.loc[31, 'h [kJ/kg]'] - df.loc[36, 'h [kJ/kg]'])

    epsilon_comp = 1 - ed_comp / ef_comp
    epsilon_cond = 1 - ed_cond / ef_cond
    epsilon_ihx = 1 - ed_ihx / ef_ihx
    if abs(df.loc[33, 's [J/kgK]'] - df.loc[34, 's [J/kgK]']) < 1e-3:  # --> ideal expander instead of valve
        ef_val = df.loc[31, 'm [kg/s]'] * (df.loc[33, 'e^PH [kJ/kg]'] - df.loc[34, 'e^PH [kJ/kg]'])
        epsilon_val = 1 - ed_val / ef_val
    else:  # --> real valve
        if df.loc[34, 'T [°C]'] > (t0-273.15):  # --> dissipative valve
            epsilon_val = np.nan
            ef_val = np.nan
        else:
            ...
            ef_val = (e33t + e33m - e34m) * df.loc[31, 'm [kg/s]']
            epsilon_val = e34t / (e33t + e33m - e34m)
    
    heat_eva = df.loc[31, 'm [kg/s]'] * (df.loc[35, 'h [kJ/kg]'] - df.loc[34, 'h [kJ/kg]'])
    heat_cond = df.loc[21, 'm [kg/s]'] * (df.loc[22, 'h [kJ/kg]'] - df.loc[21, 'h [kJ/kg]'])
    heat_ihx = df.loc[31, 'm [kg/s]'] * (df.loc[36, 'h [kJ/kg]'] - df.loc[35, 'h [kJ/kg]'])
    power_comp = df.loc[31, 'm [kg/s]'] * (df.loc[31, 'h [kJ/kg]'] - df.loc[36, 'h [kJ/kg]'])
    power_cond = (df.loc[31, 'm [kg/s]'] * (df.loc[31, 'h [kJ/kg]'] - df.loc[32, 'h [kJ/kg]'])
                  + df.loc[21, 'm [kg/s]'] * (df.loc[21, 'h [kJ/kg]'] - df.loc[22, 'h [kJ/kg]']))
    power_ihx = df.loc[31, 'm [kg/s]'] * (df.loc[32, 'h [kJ/kg]'] - df.loc[33, 'h [kJ/kg]'] +
                                            df.loc[35, 'h [kJ/kg]'] - df.loc[36, 'h [kJ/kg]'])
    power_val = df.loc[31, 'm [kg/s]'] * (df.loc[33, 'h [kJ/kg]'] - df.loc[34, 'h [kJ/kg]'])
...
def perform_adex_hp(p32_start, config, label, df_ed, adex=False, save=False, print_results=False, calc_epsilon=False, output_buffer=None):
...
def conv_exergy_analysis():
    ...
    p63_opt = find_opt_p32(p63_start, target_diff, config=config_real, label=label_real, adex=False)
