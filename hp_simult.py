...
from func_fix import (pr_func, pr_deriv, eta_s_PUMP_func, eta_s_PUMP_deriv, turbo_func, turbo_deriv, he_func, he_deriv,
                      ...
                      eps_he_func, eps_he_deriv, eps_ihx_func, eps_ihx_deriv)

pd.set_option('display.max_columns', None)
pd.set_option('display.width', None)


def hp_simultaneous(target_p32, print_results, case, adex=False, id_comp=False, id_ihx=False, plot=False, delta_t_min=5):
    ...
    while np.linalg.norm(residual) > 1e-4:
        ...
        #   0
        if adex and id_comp:
            t32_calc_comp = eps_compressor_func(1, variables[0], variables[10], variables[1], variables[2], wf)
        elif adex and not id_comp:
            t32_calc_comp = eps_compressor_func(epsilon['COMP'], variables[0], variables[10], variables[1], variables[2], wf)
        else:
            t32_calc_comp = eta_s_PUMP_func(eta_s, variables[0], variables[10], variables[1], variables[2], wf)
        #   1
        if adex and id_ihx:
            t36_set = eps_ihx_func(1, variables[3], target_p32, variables[8], variables[11], wf,
                                   variables[9], variables[15], variables[0], variables[10], wf)
        elif adex and not id_ihx:
            t36_set = eps_ihx_func(epsilon['IHX'], variables[3], target_p32, variables[8], variables[11], wf,
                                   variables[9], variables[15], variables[0], variables[10], wf)
        else:
        ...
        residual = np.array([t32_calc_comp, t36_set, t32_set, p31_set, power_calc_comp, m31_calc_cond, t21_set, t22_set,
                             t33_calc_ihx, p36_set, p33_set, t34_calc_valve, p35_calc_eva, t34_set, p22_set, p35_set, 
        ...
        #   0
        if adex and id_comp:
            t32_calc_comp_j = eps_compressor_deriv(1, variables[0], variables[10], variables[1], variables[2], wf)
        elif adex and not id_comp:
            t32_calc_comp_j = eps_compressor_deriv(epsilon['COMP'], variables[0], variables[10], variables[1], variables[2], wf)
        else:
            t32_calc_comp_j = eta_s_PUMP_deriv(eta_s, variables[0], variables[10], variables[1], variables[2], wf)
        #   1
        if adex and id_ihx:
            t36_set_j = eps_ihx_deriv(1, variables[3], target_p32, variables[8], variables[11], wf,
                                   variables[9], variables[15], variables[0], variables[10], wf)
        elif adex and not id_ihx:
            t36_set_j = eps_ihx_deriv(epsilon['IHX'], variables[3], target_p32, variables[8], variables[11], wf,
                                   variables[9], variables[15], variables[0], variables[10], wf)
        else:
        ...
        jacobian[0, 0] = t32_calc_comp_j["h_1"]  # derivative of t32_calc_comp with respect to h36
        jacobian[0, 10] = t32_calc_comp_j["p_1"]  # derivative of t32_calc_comp with respect to p36
        jacobian[0, 1] = t32_calc_comp_j["h_2"]  # derivative of t32_calc_comp with respect to h31
        jacobian[0, 2] = t32_calc_comp_j["p_2"]  # derivative of t32_calc_comp with respect to p31
        if adex:
...
def find_opt_p32(p32_opt_start, min_t_diff_cond_start, target_min_td_cond):
    ...
    while diff > tolerance:
        ...
        [_, min_t_diff_cond, cop] = hp_simultaneous(p32_opt, print_results=False, case='optimal base')
...
# IDEAL COMP
[_, min_t_diff_cond, _] = hp_simultaneous(p32_base_opt, print_results=True, case='id_COMP', adex=True, id_comp=True)
p32_id_COMP_opt = find_opt_p32(p32_base_opt, min_t_diff_cond, 5)
[_, _, _] = hp_simultaneous(p32_id_COMP_opt, print_results=True, case='optimal id_COMP', adex=True, id_comp=True)

# IDEAL IHX
[_, min_t_diff_cond, _] = hp_simultaneous(p32_base_opt, print_results=True, case='id_COMP_IHX', adex=True, id_ihx=True)
p32_id_IHX_opt = find_opt_p32(p32_base_opt, min_t_diff_cond, 5)
[df_IHX, _, _] = hp_simultaneous(p32_id_IHX_opt, print_results=True, case='optimal id_COMP_IHX', adex=True, id_ihx=True)
