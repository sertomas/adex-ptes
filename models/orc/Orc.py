...
class Orc:
    """
    A class representing an Organic Rankine Cycle (ORC) system.

    Args:
        config (dict): Configuration parameters for the ORC system.
        params (dict): Parameters defining the ORC components and properties.

    Attributes:
        working_fluid (str): The working fluid used in the ORC.
        water (str): The secondary fluid (typically water) used in the ORC.
        working_fluid_vec (dict): A dictionary specifying the composition of the working fluid.
        water_vec (dict): A dictionary specifying the composition of the secondary fluid.
        comps (dict): A dictionary to store components of the ORC system.
        conns (dict): A dictionary to store connections in the ORC system.
        busses (dict): A dictionary to store busses in the ORC system.
        network (Network): An instance of the Network class for modeling the ORC system.
        eta (float): The efficiency of the ORC system (initialized to NaN).
        epsilon (float): The exergy efficiency of the ORC system (initialized to NaN).

    Methods:
        generate_components: Initialize components of the ORC system.
        ...
        perform_sens_anal_p_high: Perform a sensitivity analysis of the high pressure of the ORC system.
        get_states: Get states to generate log(p)-h-diagram of the ORC system.
        perform_exergy_analysis: Perform exergy analysis.
        plot_logph: Generate log(p)-h-diagram of the ORC system.
        qt_diagram: Generate a QT diagram of the ORC system.

    """

    # Constructor to initialize the Orc object with configuration and parameters.
    def __init__(self, config, params):
        """
        Initialize a Orc object with the given configuration and parameters.

        Args:
            config (dict): Configuration settings for the ORC system.
    ...
    # Generate and plot log(p)-h-diagram of the ORC process.
    def plot_logph(self, result_dict, path=None, return_diagram=False):
        """
        Generate and plot log(p)-h-diagram of the ORC process.
        ...
        # Draw ORC system over fluid property diagram
        for compdata in result_dict.values():
    ...
    # Generate and plot T-s-diagram of the ORC system.
    def plot_Ts(self, path=None, return_diagram=False):
        """
        Generate and plot T-s-diagram of ORC system.
    ...
    # Generate a QT diagram for the ORC component.
    def qt_diagram(self, hot_in, hot_out, cold_in, cold_out, delta_t_min, case, plot=False, path=None):
        """
        Generate a Q-T (Heat vs. Temperature) diagram for a heat exchanger in the ORC (Organic Rankine Cycle).

        Args:
            hot_in (int): The connection number of the hot fluid inlet.
            hot_out (int): The connection number of the hot fluid outlet.
            cold_in (int): The connection number of the cold fluid inlet.
            cold_out (int): The connection number of the cold fluid outlet.
            delta_t_min (float): The minimum temperature difference allowed in the heat exchanger (in Kelvin).
            plot (bool, optional): Whether to display the generated diagram. Default is False.
            path (str, optional): The file path to save the diagram. Default is None.

        Returns:
            list: A list containing the minimum and maximum temperature differences in the heat exchanger.
        """
        ...
        plt.title("QT diagram of the " + component_name + f" of the ORC \nfor the case: {case}")
        ...
        if min(difference) > delta_t_min - 1e-3:
            print("The min. temperature difference in the " + component_name + " of the ORC is " + str(round(min(difference), 2)) + "K and is equal or higher than the allowed min. (" + str(
                delta_t_min) + "K).")
        else:
            print("The min. temperature difference in the " + component_name + " of the ORC is " + str(round(min(difference), 2)) + "K and is lower than the allowed min. (" + str(
                delta_t_min) + "K).")
