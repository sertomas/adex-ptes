...
def plot_exergy_destruction(ax, df, name_mapping, components, title, colors, labels):
    ...
    for i, totals in enumerate(totals_positive):
        ...
        for j, total in enumerate(totals):
            if total > 0:
                ax.barh(components_mapped[i], total, left=left, color=colors[j], edgecolor="black", linewidth=1, height=0.9)
                left += total
    ...
    # Add text to the right top corner of the plot
...
labels = [r'$\dot{E}^\mathrm{EN}_{\mathrm{D},k}$', r'$\dot{E}^\mathrm{MX}_{\mathrm{D},k}$', r'$\sum_{l\neq k}\dot{E}^{\mathrm{EX},kl}_{\mathrm{D},k}$']
