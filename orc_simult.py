...
def orc_simultaneous(target_p63, print_results, config, label, adex=False, plot=False):
    ...
    while np.linalg.norm(residual) > 1e-4:
        ...
        # 7
        if adex and config["eva"]:
            t64_set = eps_real_he_func(epsilon['eva'], variables[11], p51, variables[12], variables[13], m51, fluid_tes,
        ...
        if adex and not config["ihx"]:
            t63_calc_ihx = ideal_ihx_entropy_func(variables[9], variables[8], variables[3], variables[4], wf,
                                                  variables[2], variables[5], variables[10], target_p63, wf)
        else:
        ...
        if adex and not config["pump"]:
            ...
            t62_calc_pump_j = eta_s_compressor_deriv(eta_s_pump, variables[0], variables[1], variables[2], variables[5], wf)        # 3
        # 3
        if adex and config["ihx"]:
        ...
        # 7
        if adex and config["eva"]:
            t64_set_j = eps_real_he_deriv(epsilon['eva'], variables[11], p51, variables[12], variables[13], m51, fluid_tes,
                                          variables[10], target_p63, variables[7], variables[6], variables[14], wf)
        else:
        ...
        # 10
        if adex and not config["ihx"]:
            t63_calc_ihx_j = ideal_ihx_entropy_deriv(variables[9], variables[8], variables[3], variables[4], wf,
                                                     variables[2], variables[5], variables[10], target_p63, wf)
        else:
        ...
        if adex and config["ihx"]:
            jacobian[3, 9] = t66_set_j["h_hot_in"]  # derivative of t63_calc_ihx_j with respect to h65
            jacobian[3, 8] = t66_set_j["p_hot_in"]  # derivative of t63_calc_ihx_j with respect to p65
            jacobian[3, 3] = t66_set_j["h_hot_out"]  # derivative of t63_calc_ihx_j with respect to h66
            jacobian[3, 4] = t66_set_j["p_hot_out"]  # derivative of t63_calc_ihx_j with respect to p66
            jacobian[3, 2] = t66_set_j["h_cold_in"]  # derivative of t63_calc_ihx_j with respect to h62
            jacobian[3, 5] = t66_set_j["p_cold_in"]  # derivative of t63_calc_ihx_j with respect to p62
            jacobian[3, 10] = t66_set_j["h_cold_out"]  # derivative of t63_calc_ihx_j with respect to h63
        else:
        ...
        if adex and config["eva"]:
            jacobian[7, 11] = t64_set_j["h_hot_in"]  # derivative of m61_calc_eva with respect to h51
            jacobian[7, 12] = t64_set_j["h_hot_out"]  # derivative of m61_calc_eva with respect to h52
            jacobian[7, 13] = t64_set_j["p_hot_out"]  # derivative of m61_calc_eva with respect to p52
            jacobian[7, 10] = t64_set_j["h_cold_in"]  # derivative of m61_calc_eva with respect to h63
            jacobian[7, 7] = t64_set_j["h_cold_out"]  # derivative of m61_calc_eva with respect to h64
            jacobian[7, 6] = t64_set_j["p_cold_out"]  # derivative of m61_calc_eva with respect to p64
            jacobian[7, 14] = t64_set_j["m_cold"]  # derivative of m61_calc_eva with respect to m61
        else:
        ...
        if adex and not config["ihx"]:
            jacobian[10, 9] = t63_calc_ihx_j["h_hot_in"]  # derivative of t63_calc_ihx_j with respect to h65
            jacobian[10, 8] = t63_calc_ihx_j["p_hot_in"]  # derivative of t63_calc_ihx_j with respect to p65
            jacobian[10, 3] = t63_calc_ihx_j["h_hot_out"]  # derivative of t63_calc_ihx_j with respect to h66
            jacobian[10, 4] = t63_calc_ihx_j["p_hot_out"]  # derivative of t63_calc_ihx_j with respect to p66
            jacobian[10, 2] = t63_calc_ihx_j["h_cold_in"]  # derivative of t63_calc_ihx_j with respect to h62
            jacobian[10, 5] = t63_calc_ihx_j["p_cold_in"]  # derivative of t63_calc_ihx_j with respect to p62
            jacobian[10, 10] = t63_calc_ihx_j["h_cold_out"]  # derivative of t63_calc_ihx_j with respect to h63
        else:
...
df_components.round(4).to_csv(f'outputs/adex_orc/orc_comps_real.csv')

[config_ideal, label_ideal] = set_adex_orc_config('pump', 'eva', 'exp', 'ihx', 'cond')
p63_start = p63_opt  # bar
